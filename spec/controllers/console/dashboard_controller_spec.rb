  ...
  describe 'GET #index' do
    context 'when the user is authorized' do
      ...
      it 'assigns the user\'s categories to @categories' do
        get :index, params: { account_id: account.id }
        expect(assigns(:categories)).to eq(account.categories)
      end

      it 'assigns the correct transactions to @transactions based on the query' do
        get :index, params: { account_id: account.id, q: { date_gteq: '2023-01-01', date_lteq: '2023-12-31' } }
        expect(assigns(:transactions)).to eq(account.transactions.where(date: '2023-01-01'..'2023-12-31'))
      end
    ...
    context 'when the user is not authorized' do
      ...
      it 'raises a Pundit::NotAuthorizedError' do
        expect { get :index, params: { account_id: account.id } }.to raise_error(Pundit::NotAuthorizedError)
      end
    end

    context 'when a year is passed in the query params' do
      it 'sets the date range for the entire year' do
        get :index, params: { account_id: account.id, q: { date_eq: '2021' } }
    ...
    it 'handles categories with transactions in some months correctly' do
      category = create(:category, account: account)
      create(:transaction, account: account, category: category, date: Date.parse('2023-01-15'))
      create(:transaction, account: account, category: category, date: Date.parse('2023-06-15'))
      get :index, params: { account_id: account.id }
      assigns(:category_data).each do |category_id, data|
    ...
    describe '#build_category_data' do
      let(:category1) { create(:category, account: account, name: "Cibo") }
      let(:category2) { create(:category, account: account, name: "Auto") }
